...
#   conn.row_factory = psycopg2.Row
    ...
    cur = conn.cursor()
    cur.execute('''SELECT distinct domain, filetype, Count(*) FROM afqsn group by domain, filetype order by domain''')
    rows = cur.fetchall();
    print()
    conn.close()
    return render_template("domainmalicioussamples.html", rows=rows)
